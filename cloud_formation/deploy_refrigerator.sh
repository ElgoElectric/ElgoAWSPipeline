...
S3_BUCKET=$(echo "elgo-rf-ad-v6-$AWS_ACCOUNT_ID-$AWS_REGION-${ENVIRONMENT}" | tr '[:upper:]' '[:lower:]')
...
INGEST_PIPELINE_STACK_NAME=elgo-rf-ad-v6${ENVIRONMENT}
...
echo "Deploying refrigerator ingest pipeline"
sam package \
	--template-file ./ElgoRefrigerator-Pipeline.yaml \
	--s3-bucket $S3_BUCKET \
	--output-template-file ./ElgoRefrigerator-Pipeline-Packaged.yaml $AWS_ARGS
sam deploy \
	--template-file ./ElgoRefrigerator-Pipeline-Packaged.yaml \
