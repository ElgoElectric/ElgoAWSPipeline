...
S3_BUCKET=$(echo "elgo-ka2-ad-v1-$AWS_ACCOUNT_ID-$AWS_REGION-${ENVIRONMENT}" | tr '[:upper:]' '[:lower:]')
...
INGEST_PIPELINE_STACK_NAME=elgo-ka2-ad-v1${ENVIRONMENT}
...
sam package \
	--template-file ./ElgoKA2-Pipeline.yaml \
	--s3-bucket $S3_BUCKET \
	--output-template-file ./ElgoKA2-Pipeline-Packaged.yaml $AWS_ARGS
sam deploy \
	--template-file ./ElgoKA2-Pipeline-Packaged.yaml \
